...
**Master scratch block compression — the dRGT `/gtmp` ENOSPC fix (VALIDATED).** The master's MR expression scratch (`.sc0`) is compressed on disk by a **master-local block-compression layer** in [sources/tools.c](sources/tools.c) (the `bc_*` functions): it intercepts `WriteFileToFile`/`ReadFile`/`SeekFile` for the master's tracked scratch fd, compresses each `POsize` buffer flush as one independent `zlib` block (`compress2`), and serves windowed reads from a RAM logical→physical block index. This deliberately AVOIDS FORM's continuous-stream `SetupOutputGZIP` (the path the 2013 parform disable guards, which broke the master EndSort when re-enabled). The hook is in `PF_EndSort` ([parallel.c](sources/parallel.c)): `PF_bc_track(fout)` registers the scratch **FILEHANDLE pointer** (NOT `fout->handle` — the file is created lazily so `->handle` is `-1` there; resolve it live at write time), and `PF_bc_compress_now` gates per-expression on `!dobracketindex && !hide`. Opt-in via `PF_SCRATCH_COMPRESS=<1-9>`; unset = provable no-op. **Validated byte-identical on dRGT trunc64 brakV (sha `7638118f`).** Controlled same-node comparisons (5× zeus_new_q, no mergers, all byte-identical): **off 4641 s / 380.34 GB; zlib-1 6170 s / 15.52 GB (24.5×); zlib-6 ~7989 s / 6.26 GB (60.7×); ZSTD-1 4286 s / 3.82 GB (99.5×).** **USE ZSTD LEVEL 1 — it dominates: better ratio than zlib *level 6* (99.5× vs 60.7×) AND ~8% FASTER than running uncompressed.** With zstd the speed/space tradeoff vanishes — writing+reading 3.8 GB instead of 380 GB saves more I/O than zstd-1's cheap CPU, so **compression is net-FREE (actually faster)**. (zlib levels cost the master CPU: zlib-1 +27%, zlib-6 +72%, the master deflating ~565 GB logical scratch single-threaded; zstd erases that. My earlier "+3.2%" was combined-queue contention noise — see [[project_zeus_node_contention]].) zlib stays the no-extra-dependency fallback. So the dRGT `/gtmp` ENOSPC wall is solved (full-Nmax >1 TB → ~10 GB at zstd-1). Proof harness: `runs/Adquanta/dRGT_trunc64_brakV_bcproof.pbs`; the `[bc] ... logical=L phys=P` counter (log + `PF_BC_STATS` file) is the DIRECT proof compression engaged — byte-identity alone can't distinguish compressed from stored-raw. **Committed `3205321`** (tools.c bc layer + declare.h + parallel.c hook); the default `~/bin/parform` is a clean build of it (md5 `1ef019faaca8`, compression OFF until `PF_SCRATCH_COMPRESS` is set). **The committed bc layer uses zstd automatically when built `--with-zstd`** — already wired: bundled `extern/zstd` + the zlibWrapper in `Makefile.am` + `ZWRAP_useZSTDcompression(1)` at `startup.c:1525`, so `compress2`/`uncompress` route through ZWRAP → zstd (no bc code change). `--with-zstd` makes zstd global (reducers' sort files too, not just `.sc0`). **VALIDATED (job 4247674): `~/bin/parform.withzstd` (committed 3205321 + `--with-zstd`, md5 `2f4b9361fb2e`) on dRGT trunc64 brakV — byte-identical, `.sc0` 3.82 GB (99.5×, phys bytes identical to the direct-ZSTD run → ZWRAP maps `PF_SCRATCH_COMPRESS=1` to exactly zstd-1), −20% vs uncompressed. So productionizing zstd = just build `--with-zstd`** (the direct-ZSTD `parform.blockcompress.zstd` was a quick A/B only). See `project_zstd_productionization`. Full record: auto-memory `project_block_compression_validated`.
...
| `PF_SCRATCH_COMPRESS` | 0 (off) | 9 | compression level for **master-local `.sc0` block compression** (`tools.c` `bc_*`). 0 = off (provable no-op); 1-9 = on. See "Master scratch block compression" below. |
| `PF_SCRATCH_CODEC` | zlib | — | codec for the above: `zstd` (the winner — 99.5×, net-free) vs zlib (default). **`zstd` only works in the experiment binary `parform.blockcompress.zstd`**; the committed default is zlib-only until zstd is productionized. |
| `PF_BC_STATS` | unset | — | If set, the `.sc0` compression layer appends a per-module `logical/phys` ratio line to this file (login-visible proof). |
...
| `sources/tools.c` | Low-level file I/O (`WriteFileToFile`/`ReadFile`/`SeekFile`); **master `.sc0` block-compression `bc_*` layer** + `PF_bc_track`/`PF_bc_compress_now` |
