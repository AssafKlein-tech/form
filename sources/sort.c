	...
	if ( PF_LowMRsort() || (PF.me != MASTER && AR.sLevel <= 0 && (fi == AR.outfile || fi == AR.hidefile) && PF.parallel && PF.exprtodo < 0 )) {
		if (PF.me < PF.nummappers && AC.sMRflag != NO_MAPREDUCE){
			for (int i = PF.nummappers; i < PF.numtasks; i++){
				...
				/* NOTE: empty-buffer skip on patch=1 was attempted (commit
				   9b44b2f) but caused MPI_ERR_TYPE on the master/reducer side
				   (jobs 4039614/4039616 crashed in PF_EndSort). The shuffle
				   protocol needs the no-op send to keep request slots aligned
				   between mapper and reducer; skipping breaks that. */
