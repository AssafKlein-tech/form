...
static void pf_hash_histogram(PHEAD WORD *term)
{
	...
	WORD *start = term + 1;
	WORD *symend = pf_hash_symend(BHEAD term);
...
#else
		h = hash_list_scalar(start, (WORD)n);
#endif
		...
		if ( i < 0 ) {			/* Compressed term */
			...
			if ( lowmr_sort ) {
...
#ifdef DEBUGGING
				pf_hash_histogram(BHEAD term);  /* no-op unless PF_HASH_HISTOGRAM=1 */
#endif
				WORD *start = term + 1;
				...
				/* AM.MR.HashPrefixWords=K (default 0 = the whole range above):
				   ...
				   Truncating only coarsens routing, so the invariant still holds:
				   Compare1-equal => same hashed range => same prefix => same
				   reducer => coeffs (and PolyFun args) still summed there. */
