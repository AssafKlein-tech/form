...
An **optional** layer between reducers and the master, enabled with `PF_MERGERS=G` (env, default 0 = off). The lowest `G` mapper ranks (`1..G`) overlay a third role: after finishing their normal mapper phase (generator → local sort → shuffle to reducers → "done" to master), each runs a **second merge pass** over a group of leaf reducers and forwards one merged stream to the master. The master's loser-tree fan-in shrinks from `R` reducers to `G` mergers.

- **Assignment:** leaf reducer `r` feeds merger `1 + ((r − M) % G)` (round-robin). `PF.merger_parent` = that merger rank on a reducer, `MASTER` otherwise.
...
- **`pf_loser_src_to_rank(src)`** is the single src→MPI-rank rule for the loser tree — a merger's leaves map through `PF.merger_leaf_ranks[]`; the master's leaves (with the tier on) are the merger ranks `1..G`.
...
**Status:** the merger role (Phase 1) is committed on `MRmpi`; the drain-on-merger, the `PF_InitTree` fix, and the `MER_*` profiler phases are uncommitted on the working tree. Binary: `~/bin/parform.mergerdrain`. See `project_merger_wip.md`.
