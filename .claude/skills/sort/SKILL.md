...
```
In `PutOut()` for a reducer, `PF_LowMRsort()` is **false** (reducer rank >= nummappers), so the non-MR slave path applies: output goes to `PF.sbufs[MASTER]` → forwarded to master (or to its merger — see below).

### Merger workers (mapper-as-merger tier)

An **optional** layer between reducers and the master, enabled with `PF_MERGERS` (env, default 0 = off; any value > 0 is the on switch — the number is *not* the merger count). Placement is **always node-local**: exactly one merger per physical (shared-memory) node, hosted on that node's lowest non-master mapper rank. After finishing their normal mapper phase (generator → local sort → shuffle to reducers → "done" to master), each merger runs a **second merge pass** over its own node's leaf reducers and forwards one merged stream to the master. The master's loser-tree fan-in shrinks from `R` reducers to `G` mergers, and the reducer→merger transfer stays **intra-node** (UCX `sysv` shm, off the NIC).

- **Assignment:** leaf reducer `r` feeds the merger on `r`'s physical node. The merger count `G` is **derived** = number of reducer-bearing nodes (the `PF_MERGERS` numeric value is ignored beyond on/off). Node layout is discovered once in `PF_LibInit` (`pf_discover_node_topology`, `MPI_Comm_split_type(MPI_COMM_TYPE_SHARED)` → `PF.rank_node`, `PF.numnodes`). `PF.merger_parent` = that merger rank on a reducer, `MASTER` otherwise. If node-local placement cannot be realised (topology discovery failed, or a reducer-bearing node has no spare mapper to host a merger) the tier is **disabled for that run** — reducers feed the master directly (one master warning line, run still completes correctly).
- **Entry:** `PF_Processor` calls `PF_MergerLoop()` right after the mapper's done-send. No barrier needed — a leaf reducer's `EndSort` cannot finish until *all* mappers (including this rank) have sent `ENDSHUFFLE`, so the merger's own mapper phase is always complete before any leaf-reducer data arrives.
- **Reuses the master merge body:** `PF_MergerLoop` is a ~50-line wrapper — `NewSort` → set `PF.in_merger_phase=1` → `PF_allocateSbuf` (redirects `fout` into the sbuf) → `EndSort` (→ `PF_EndSort`, the shared loser-tree merge) → restore `fout` → clear the flag. It calls `EndSort` (the sort.c wrapper), **not** `PF_EndSort` directly — only `EndSort` does the `AR.sLevel--` cleanup, and leaking `sLevel` corrupts the next module's sort context.
- **`PF.in_merger_phase`** gates all merger-specific routing: `PF_LowMRsort()` returns false (a merger does not hash-route), `PutOut`/`FlushOut` send to `PF.merger_parent`, and `PF_WISendSbuf` passes `BUFFER`/`ENDBUFFER` tags through unchanged (not `SHUFFLE`-translated as for a mapper).
- **`pf_loser_src_to_rank(src)`** is the single src→MPI-rank rule for the loser tree — a merger's leaves map through `PF.merger_leaf_ranks[]`; the master's leaves (with the tier on) map through `PF.merger_child_ranks[]` (the merger ranks ascending, scattered one per node — built on the master in `PF_Processor`).
- **Drain:** the K-prefix drain runs inside the merger phase too — `pf_emit_compressed_bulk` branches on `in_merger_phase` and MPI-sends the merged run upstream instead of `WriteFile`-to-disk. The drain invariant (each K-prefix bucket lives entirely in one input stream) holds for a merger's reducer group exactly as it does for the master.

Data flow: `mapper → reducer → merger → master` (vs `mapper → reducer → master` at `G=0`, which is bit-for-bit the original path).
